...
// Revision: $Id: //dwh/DW_ocb/DW_apb_i2c/amba_dev/src/DW_apb_i2c.v#23 $ 
//
//
// File    : DW_apb_i2c.v
//
...
//LJ : paddr[0] is used to select lower byte of the 16-bit data word register.
...
// LJ : Dummy wires - used to suppress unconnected ports warnings by Leda tool - BR - 2/16/2010
...
//#
   ...
   // LJ : tx_abrt_source 6,8 may be assigned zero if the corresponding configuration si not present.
   // leda NTL_CON10B off
   // LMD: Output tied to supply in design
   // LJ : tx_abrt_source 6,8 may be assigned zero if the corresponding configuration si not present.
   i_i2c_DW_apb_i2c_toggle
    U_DW_apb_i2c_toggle (
      //inputs
      .ic_rst_n(ic_rst_n),
                                          .ic_clk(ic_clk),
   ...
   // leda NTL_CON10 off
   // LMD: output tied to supply
   // LJ : ic_raw_intr_stat signal bits 12, 13 may be assigned zero if the corresponding interrupt features are turned off.
   // leda NTL_CON10B off
   // LMD: Output tied to supply in design
   // LJ : ic_raw_intr_stat signal bits 12, 13 may be assigned zero if the corresponding interrupt features are turned off.
   i_i2c_DW_apb_i2c_intctl
    U_DW_apb_i2c_intctl
     (
      // APB bus interface
      .pclk(pclk),
   ...
   i_i2c_DW_apb_i2c_bcm57
    #(`IC_DATA_FIFO_RS, `IC_RX_BUFFER_DEPTH, 0, `RX_ABW) U_dff_rx (
     .clk                  (pclk),
     .rst_n                (presetn),
     .init_n		   (1'b1),
     .wr_addr              (rx_wr_addr),
   ...
   i_i2c_DW_apb_i2c_bcm57
    #(`IC_DATA_TX_CMD_RS, `IC_TX_BUFFER_DEPTH, 0, `TX_ABW) U_dff_tx (
     .clk                  (pclk),
     .rst_n                (presetn),
     .init_n		   (1'b1),
     .wr_addr              (tx_wr_addr),
