...
# Revision: $Id: //dwh/DW_ocb/bin/amba_dev/pkg/perl/runtest.pm#26 $ 
# ---------------------------------------------------------------------------
# File:         runtest.pm
...
push (@ExportedSubs, qw(&generateVrl &generateVshell &runSim &runRalSim &postProcess));
@EXPORT = @ExportedSubs;


use subs qw(&sourceLicenseCheck &fetchModuleName &runDwVipSetup &compile_exit &runtestPreparation &convertPath &parseCcConstants &commonVerilogDefines &commonVerilogIncludes &generateVeraPlusargs &generateCcConstants &compileCheckEnvironment &compileVera &generate_vro &add_include_list &get_file_list_by_ext &generateVrl &generateVshell &generate_proj &runSim &runRalSim &my_system &postProcess &check_completed &check_errors &check_illegal_prog &check_timeouts &act_cert_requested &act_certified &act_recert_requested &act_recertified);
...
sub runDwVipSetup {
   ...
   if(-f "../scratch/.dw_vip.cfg")
   {
      ...
      system("grep '^%MOD' ../scratch/.dw_vip.cfg | sed 's:%MOD ::' | sed 's:amba:-v:' | sed 's: \$::' | sed 's: mmt : -v :' | sort -k 1 > vmt_model_list.tmp");

      # compare with the actual model list file
      if(system("diff -b vera_version.tmp .vera_version_prev  > vera_version.diff"))
      {
         ...
         system("which vera > .vera_version_prev");
         $main::VeraForceCompile = 1;
      }
      elsif(system("diff -b vmt_model_list.tmp vmt_model_list > vmt_model_list.diff"))
      {
         printf STDERR "calling dw_vip_setup to refresh headers in ../scratch \n";
         system("$designware_home/bin/dw_vip_setup -path ../scratch -vmt $vmt_version -nos -a -m vmt_model_list");
         system("which vera > .vera_version_prev");
...
sub runtestPreparation() {
  ...
  $main::ccparams_file = "${main::coreKit}/src/${main::DesignName}_cc_constants.v";
...
if (($main::Simulator eq "vcs") or ($main::Simulator eq "mti_verilog") or ($main::Simulator eq "verilog_xl") or ($main::Simulator eq "nc_verilog") or ($main::Simulator eq "vcsi")) {
  ...
  # jstokes, 2.6.11, STAR 9000463283 & 9000466641
  # Need to control when exclusive accesses are generated due to work around required for
  # VIP master bug
  if($main::DesignName eq "DW_axi_gs") {
    if($main::Test eq "test_exclusive") {
      printf SCF "+define+GS_TEST_EXCLUSIVE\n";
    }
  }
  ...
  if($main::Simulator eq "vcs") {
    ...
    # Always turn on SV for some components (Those that use SV elements in the testbench).
    if(  ($main::DesignName eq "DW_axi_gm")
       | ($main::DesignName eq "DW_axi_gs")
       | ($main::DesignName eq "DW_ahb_dmac")
       | ($main::DesignName eq "DW_apb_i2c")
      )
    {
      printf SCF "-sverilog\n";
      printf SCF "-assert dve\n";
    # Otherwise only turn on SV if specificaly asked.
    } elsif ($main::SystemVerilogCompile) {
      printf SCF "-sverilog\n";
    }
  }
...
#  printf SCF "+nowarnLIBNOU\n";
  ...
  if($main::Simulator eq "mti_verilog") {
    printf SCF "+define+MTI\n";
  }
  if($main::Simulator eq "nc_verilog") {
    printf SCF "+define+NC_VERILOG\n";
  }

  # common plusargs for DW_axi_x2x
  if($main::DesignName eq "DW_axi_x2x") {
    if($main::ccparams{'X2X_CLK_MODE'} eq 1) {
      if(($main::ccparams{'X2X_MP_SYNC_DEPTH'} eq 0) && ($main::ccparams{'X2X_SP_SYNC_DEPTH'} eq 0)) {
        printf SCF ("+define+PERIOD_M=6\n");
        printf SCF ("+define+PERIOD_S=12\n");
      } elsif($main::ccparams{'X2X_MP_SYNC_DEPTH'} gt $main::ccparams{'X2X_SP_SYNC_DEPTH'}) {
        printf SCF ("+define+PERIOD_M=6\n");
        printf SCF ("+define+PERIOD_S=13\n");
      } else {
        printf SCF ("+define+PERIOD_M=13\n");
        printf SCF ("+define+PERIOD_S=6\n");
      }
    } else {
      printf SCF ("+define+PERIOD_M=6\n");
      printf SCF ("+define+PERIOD_S=6\n");
    ...
    if($main::SVCoverage) {
      printf SCF ("+define+X2X_SVA_ENABLE\n");
  ...
  if ($main::DesignName eq "DW_ahb_dmac") {
    ...
    # AARAUJO, 06/05/2010, v2k is switched on by default so that no need to define it again
    #printf SCF ("+v2k\n");
    ...
    if($main::SystemVerilogCompile) {
      printf SCF ("+define+DMAC_SVA_ENABLED\n");
    }
  }
 ...
 # -- SDF annotation
  ...
  if ($main::RndSeed) {
    printf SCF "// Random Seed for Test case Seeding\n";
    printf SCF "+define+__RandomSeed__=$main::RndSeed\n";
  }
  # -- Dump file control
  if ($main::DumpEnabled) {
...
sub commonVerilogIncludes($) {
  ...
  elsif ($main::DesignName eq "DW_axi_x2x") {
    printf RCB "-f $main::TestbenchDir/axi.lst\n\n";
    printf RCB "+incdir+$main::TestbenchDir/axi/src\n\n";
    printf RCB "-y $main::TestbenchDir/axi/src\n\n";
  }
...
sub generateVeraPlusargs ($) {
  ...
  if($Design eq "DW_axi_gm") {
      printf VPF "+cc_filename=../../src/DW_axi_gm_cc_constants.v\n";
  } elsif($Design eq "DW_axi_gs") {
      printf VPF "+cc_filename=../../src/DW_axi_gs_cc_constants.v\n";
  } elsif($Design eq "DW_axi_hmx") {
      printf VPF "+cc_filename=../../src/DW_axi_hmx_cc_constants.v\n";
  } elsif($Design eq "DW_axi_x2h") {
      printf VPF "+cc_filename=../../src/DW_axi_x2h_cc_constants.v\n";
  } elsif($Design eq "DW_axi_x2x") {
      printf VPF "+cc_filename=../../src/DW_axi_x2x_cc_constants.v\n";
  }

  if ($Design eq "DW_axi_gm") {
    printf VPF "+dlevel=0\n";
    printf VPF "+low_power_support=1\n";
    printf VPF "+replay=0\n";
    printf VPF "+transactions=1000\n";
    printf VPF "+disable_protocol_check=0\n";
    printf VPF "+disable_slverr=1\n";
  }

  if ($Design eq "DW_axi_gs") {
    printf VPF "+dlevel=0\n";
    printf VPF "+low_power_support=1\n";
    printf VPF "+replay=0\n";
    printf VPF "+transactions=1000\n";
    printf VPF "+disable_protocol_check=0\n";
    if($main::Test eq "test_random") {
      printf VPF "+disable_slverr=0\n";
      printf VPF "+test_exclusive=0\n";
    }
    if($main::Test eq "test_exclusive") {
      printf VPF "+disable_slverr=0\n";
      printf VPF "+test_exclusive=1\n";
    }
  }
 ...
 printf VPF "+enable_coverage=$main::EnCovMon\n";
  ...
  if($Design eq "DW_axi_x2p") {

    # -- Plusargs for the DW_axi_apb design
...
sub generateCcConstants($) {
  ...
  # JS_DEBUG, 2/6/2008, adding 64 bit support to the icm.
  elsif ($Design eq "DW_ahb_icm") {
    ...
    while (<AHB_DEFAULT_PARAMS>) {
      ...
      if (/^\s*\`define\s+HADDR_WIDTH\s+64/) { $sixty_four_b_haddr = 1; }
   ...
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_EA_1 64'h09ffffffffffffff\n";
   }
      } elsif (/^\s*\`define\s+R1_N_SA_2\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_SA_2 64'h0a00000000000000\n";
   }
      } elsif (/^\s*\`define\s+R1_N_EA_2\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_EA_2 64'h0a0fffffffffffff\n";
   }
      } elsif (/^\s*\`define\s+R1_N_SA_3\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_SA_3 64'h0c00000000000000\n";
   }
      } elsif (/^\s*\`define\s+R1_N_EA_3\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_EA_3 64'h0c0fffffffffffff\n";
   }
      } elsif (/^\s*\`define\s+R1_N_SA_4\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_SA_4 64'h0e00000000000000\n";
   }
      } elsif (/^\s*\`define\s+R1_N_EA_4\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_EA_4 64'h0e0fffffffffffff\n";
   }
      } elsif (/^\s*\`define\s+R1_N_SA_5\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_SA_5 64'h1000000000000000\n";
   }
      } elsif (/^\s*\`define\s+R1_N_EA_5\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_EA_5 64'h100fffffffffffff\n";
   }
      } elsif (/^\s*\`define\s+R1_N_SA_6\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_SA_6 64'h1200000000000000\n";
   }
      } elsif (/^\s*\`define\s+R1_N_EA_6\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_EA_6 64'h120fffffffffffff\n";
   }
      } elsif (/^\s*\`define\s+R1_N_SA_7\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_SA_7 64'h1400000000000000\n";
   }
      } elsif (/^\s*\`define\s+R1_N_EA_7\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_EA_7 64'h140fffffffffffff\n";
   }
      } elsif (/^\s*\`define\s+R1_N_SA_8\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_SA_8 64'h1600000000000000\n";
   }
      } elsif (/^\s*\`define\s+R1_N_EA_8\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_EA_8 64'h160fffffffffffff\n";
   }
      } elsif (/^\s*\`define\s+R1_N_SA_9\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_SA_9 64'h1800000000000000\n";
   }
      } elsif (/^\s*\`define\s+R1_N_EA_9\s+/) {
   if ($sixty_four_b_haddr == 1) {
     printf AHB_OVERRIDE_PARAMS "`define R1_N_EA_9 64'h180fffffffffffff\n";
   }
      } else {
   print AHB_OVERRIDE_PARAMS $_;
      }
...
sub compileVera($) {
    ...
    if (-f "$path/../scratch/wait_for_compile") {
      $wait = 1;
      print STDERR "runtest: Waiting for compilation to finish\n";
      while ($wait == 1) {
        system("sleep 10");
        if (-f "$path/../scratch/wait_for_compile") {
          $wait = 1;
      ...
      print STDERR "runtest: Compilation has finished\n";
    }
    system("touch $path/../scratch/wait_for_compile");
    ...
    system("/bin/rm -rf $path/../scratch/wait_for_compile");
}
...
sub runSim() {
  ...
  if ($main::DesignName eq "DW_ahb_h2h" ||
      ...
      $main::DesignName eq "DW_axi_x2h" ||
      $main::DesignName eq "DW_axi_x2p") {
    $vcs_vera_bind="-vera";
  }

  #my $cm_cflags = ($main::Coverage) ? "-cm line+cond" : "";
  # - New method - replacing the old shown above.
  #my $cm_cflags = ($main::Coverage) ? "-cm line+cond -cm_noconst -cm_hier ../cm_hier_config" : "";
  #my $cm_rflags = ($main::Coverage) ? "-cm line+cond                                       " : "";
  # jstokes, 21.12.2010, adding assertion coverage for all
  my $cm_cflags;
  my $cm_rflags;

  $cm_cflags = ($main::Coverage) ? "-lca -cm line+cond+fsm+tgl+branch+assert  -cm_noconst -cm_hier ../cm_hier_config" : "";
  $cm_rflags = ($main::Coverage) ? "-lca -cm line+cond+fsm+tgl+branch+assert  -cm_noconst -cm_hier ../cm_hier_config" : "";

  if(($main::DesignName eq "DW_apb_uart") && (($main::Simulator eq "vcs") || ($main::Simulator eq "vcsi"))) {
    $cm_cflags = $cm_cflags . " +vera_mailbox_size=2048 +vera_semaphore_size=4096 ";
    $cm_rflags = $cm_rflags . " +vera_mailbox_size=2048 +vera_semaphore_size=4096 ";
  }
  ...
  my $sv_cflags = ($main::SVCoverage) ? "-sverilog -ntb_opts dtm -assert nopostproc -debug_all" : "";

  if ($main::Simulator eq "vcs" or $main::Simulator eq "vcsi") {
    my $PlatformFlags = "$sixtyfourbitswitch -Mupdate=1";
    @Commands = ("$main::Simulator -PP $PlatformFlags $cm_cflags $vcs_vera_bind $sv_cflags -f $main::simcommand -f $main::plusargs",
                 "/bin/rm -fr *.o",
                 "./simv $cm_rflags",
                 "rm -f simv");
  } elsif ($main::Simulator eq "mti_verilog") {
    if ($main::RtlSim eq "GTECH") {
        if ($main::DesignName eq "DW_axi_gs" || $main::DesignName eq "DW_axi_x2h") {
            @Commands = ("/bin/rm -rf ./work_ahb",
                         "vlib ./work_ahb",
                         "vlog -work work_ahb -f $main::simcommand -f $main::plusargs -v ../packages/gtech/src_ver/gtech_lib.v",
                         "vsim -lic_vlog -c -pli ../veriuser.so -lib ./work_ahb -do \"run -all;quit\"  test_${main::DesignName} -f $main::plusargs");
        } else {
            @Commands = ("/bin/rm -rf ./work_ahb",
                         "vlib ./work_ahb",
                         "vlog -work work_ahb -f $main::simcommand -f $main::plusargs -v $ENV{SYNOPSYS}/packages/gtech/src_ver/gtech_lib.v",
                         "vsim -lic_vlog -c -pli ../veriuser.so -lib ./work_ahb -do \"run -all;quit\"  test_${main::DesignName} -f $main::plusargs");
        }
    } else {
      @Commands = ("/bin/rm -rf ./work_ahb",
                   "vlib ./work_ahb",
                   "vlog -work work_ahb -f $main::simcommand -f $main::plusargs",
                   "vsim -lic_vlog -c -pli ../veriuser.so -lib ./work_ahb -do \"run -all;quit\"  test_${main::DesignName} -f $main::plusargs");
    }
    ...
    @Commands = ("rm -f hdl.var cds.lib ncsim.args libpli.so libpli.sl",
                 "ln -s ../libpli.so .",
                 "ln -s ../libpli.sl .",
                 "ncprep +overwrite -f $main::simcommand -f $main::plusargs",
...
sub generate_vro {
    ...
    if ( !( -f "$dest_dir/$base.vro") || ($main::VeraForceCompile == 1) || ( ($main::VeraForceCompileMain == 1) && ($base eq "test_${main::DesignName}") ) ) {
        ...
        if($main::TestSpiritRegisterMapDefn){
          $vera_cmd .= "-D TEST_SPIRIT=1 ";
        }

        if($main::FCoverage){
          $vera_cmd .= "-D FUNC_COVERAGE=1 ";
        }

        if($main::CHSUSFCoverage){
          $vera_cmd .= "-D CH_SUS_FUNC_COVERAGE=1 ";
        }

        if($main::RegressSimulationRuns){
          $vera_cmd .= "-D REGRESS_RUNS=1 ";
        } else {
          $vera_cmd .= "-D REGRESS_RUNS=0 ";
        }

        if($main::VIPUpdate){
          $vera_cmd .= "-D VIP_UPDATE=1 ";
        } else {
          $vera_cmd .= "-D VIP_UPDATE=0 ";
        }

        if ($main::MisSamples) {
          $vera_cmd .= "-D DW_MODEL_MISSAMPLES=1 ";
        }
